...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  /// 打开字形间距编辑器（先选择项目）
  Future<void> _openKerningEditorWithProjectPicker() async {
    try {
      final projects = await StorageService.loadProjects();
      if (!mounted) return;
      ...
      if (available.length == 1) {
        ...
        Navigator.push(
          context,
          WFAnimations.slideRoute(KerningEditorScreen(project: available.first)),
        );
        return;
      }
            ...
            children: [
              ...
              ...available.map((p) => ListTile(
                    ...
                    onTap: () {
                      ...
                      Navigator.push(
                        context,
                        WFAnimations.slideRoute(KerningEditorScreen(project: p)),
                      );
                    },
                  )),
              const SizedBox(height: 8),
  ...
  /// 打开字体家族生成器（先选择项目）
  Future<void> _openFontFamilyWithProjectPicker() async {
    try {
      final projects = await StorageService.loadProjects();
      if (!mounted) return;
      ...
      if (available.length == 1) {
        ...
        Navigator.push(
          context,
          WFAnimations.slideRoute(FontFamilyScreen(project: available.first)),
        );
        return;
      }
            ...
            children: [
              ...
              ...available.map((p) => ListTile(
                    ...
                    onTap: () {
                      ...
                      Navigator.push(
                        context,
                        WFAnimations.slideRoute(FontFamilyScreen(project: p)),
                      );
                    },
                  )),
              const SizedBox(height: 8),
  ...
  /// 打开字符集分析（先选择项目）
  Future<void> _openCharsetAnalysisWithProjectPicker() async {
    try {
      final projects = await StorageService.loadProjects();
      if (!mounted) return;
      if (projects.isEmpty) {
      ...
      if (available.length == 1) {
        Navigator.push(context, WFAnimations.slideRoute(CharsetScreen(project: available.first)));
        return;
      }
      showModalBottomSheet(
        context: context,
          ...
          child: Column(
            ...
            children: [
              ...
              ...available.map((p) => ListTile(
                ...
                title: Text(p.name),
                subtitle: Text('${p.glyphs.length} 个字形'),
                onTap: () { Navigator.pop(ctx); Navigator.push(context, WFAnimations.slideRoute(CharsetScreen(project: p))); },
              )),
            ],
          ),
        ),
  ...
  /// 打开自定义文本预览（先选择项目）
  Future<void> _openTextPreviewWithProjectPicker() async {
    try {
      final projects = await StorageService.loadProjects();
      if (!mounted) return;
      if (projects.isEmpty) {
      ...
      if (available.length == 1) {
        Navigator.push(context, WFAnimations.slideRoute(TextPreviewScreen(project: available.first)));
        return;
      }
      showModalBottomSheet(
        context: context,
          ...
          child: Column(
            ...
            children: [
              ...
              ...available.map((p) => ListTile(
                ...
                title: Text(p.name),
                subtitle: Text('${p.glyphs.length} 个字形'),
                onTap: () { Navigator.pop(ctx); Navigator.push(context, WFAnimations.slideRoute(TextPreviewScreen(project: p))); },
              )),
            ],
          ),
        ),
  ...
  /// 打开字形质量评分（先选择项目）
  Future<void> _openGlyphQualityWithProjectPicker() async {
    try {
      final projects = await StorageService.loadProjects();
      if (!mounted) return;
      ...
      if (available.length == 1) {
        ...
        Navigator.push(
          context,
          WFAnimations.slideRoute(GlyphQualityScreen(project: available.first)),
        );
        return;
      }
            ...
            children: [
              ...
              ...available.map((p) => ListTile(
                    ...
                    onTap: () {
                      ...
                      Navigator.push(
                        context,
                        WFAnimations.slideRoute(GlyphQualityScreen(project: p)),
                      );
                    },
                  )),
              const SizedBox(height: 8),
